...
export default function Sidebar() {
  return (
    <div
      style={{ width: "90px", backgroundColor: "#004578" }}
      className="fixed left-0 top-0 bottom-0 text-white flex flex-col items-center py-4 z-50"
    >
      {/* Logo */}
      <div className="mb-4">
...
function SidebarButton({ icon, label, active }: { icon: string; label: string; active?: boolean }) {
  const icons: Record<string, React.ReactNode> = {
    dashboard: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 5a1 1 0 011-1h4a1 1 0 011 1v12a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM14 5a1 1 0 011-1h4a1 1 0 011 1v12a1 1 0 01-1 1h-4a1 1 0 01-1-1V5z" />
      </svg>
    ),
    search: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <button
      className={`w-full flex flex-col items-center gap-1 px-2 py-2 transition-all ${
        active
          ? "bg-white text-[#004578]"
          : "text-blue-100 hover:opacity-80"
      }`}
    >
      {icons[icon]}
      <span className={`text-xs text-center ${active ? "font-semibold" : ""}`}>{label}</span>
    </button>
  );
}
